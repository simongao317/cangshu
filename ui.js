...
async function handleFileImport(file, type) {
    ...
    try {
        ...
        if (type === 'csv') {
            ...
            const { data: rows, isDoubanFormat } = await parseCSV(text);
            ...
            if (isDoubanFormat) {
                ...
                books = rows.map(row => {
                    ...
                    if (shelf === 'read' || status.includes('读过')) {
                        ownershipStatus = '未买';
                        wishlistReadingStatus = '已读';
                    } else if (shelf === 'wish' || status.includes('想读')) {
                        ownershipStatus = '未买';
                        wishlistReadingStatus = '想读';
                    }
                    
                    // 解析 pub 字段
                    let authors = '', publisher = '', publicationDate = '', price = '';
                    if (row['pub']) {
